"""
predict.py  —  Flood risk predictor (FIXED for cloud deployment)

Changes vs original:
  • Uses path_utils for absolute, deployment-safe paths
  • Falls back to FallbackFloodPredictor when .pkl files are missing
  • No silent failures — always returns a valid prediction dict
"""
...
# ── path bootstrap ──────────────────────────────────────────────────────────
_ML_DIR    = os.path.dirname(os.path.abspath(__file__))   # src/ml
_SRC_DIR   = os.path.dirname(_ML_DIR)                     # src
...
# ── Main predictor (uses trained Random Forest) ─────────────────────────────

class FloodRiskPredictor:
    """Loads trained model & scaler using absolute paths resolved at import time."""
    ...
    def predict_single(self, features_dict):
        ...
        return {
            "risk_level": level, "risk_label": info["label"],
    ...
    def predict_with_explanation(self, features_dict):
        ...
        if features_dict.get("rainfall_mm", 0) > 200:
            flags.append(f"⚠️ Very heavy rainfall: {features_dict['rainfall_mm']:.1f} mm")
        if features_dict.get("river_level_m", 0) > 10:
            flags.append(f"⚠️ River at danger level: {features_dict['river_level_m']:.1f} m")
        if features_dict.get("soil_moisture_percent", 0) > 85:
            flags.append(f"⚠️ Soil saturated: {features_dict['soil_moisture_percent']:.1f}%")
        if features_dict.get("distance_to_river_km", 99) < 1:
            flags.append(f"⚠️ Very close to river: {features_dict['distance_to_river_km']:.2f} km")
...
class FallbackFloodPredictor:
    ...
    _ACTIONS = [
        "Continue normal activities. Stay alert to weather updates.",
    ...
    def __init__(self, *args, **kwargs):
        print("⚠️  FallbackFloodPredictor active — ML model not found.")
        print("    To enable full ML predictions, commit data/models/ to GitHub.")
    ...
    def predict_single(self, features_dict):
        ...
        probs = self._make_probs(level)
        return {
            "risk_level": level, "risk_label": self._LABELS[level],
    ...
    def predict_batch(self, features_df):
        ...
        out["predicted_risk_level"] = features_df.apply(
            lambda row: self._score(row.to_dict()), axis=1)
...
def get_predictor(model_path=None, scaler_path=None):
    """
    Use this everywhere instead of directly instantiating FloodRiskPredictor.
    Returns the ML predictor if model files exist, otherwise the rule-based fallback.
    """
    try:
